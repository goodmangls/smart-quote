...
> ## 후속 정리 (2026-08-28)
>
> **지적 항목은 전부 닫혔다.** HIGH-1 · MEDIUM-2 · MEDIUM-3 · LOW-4 · LOW-5 모두 코드에서 직접 확인했고, 각 섹션에 근거를 달았다. "검토하지 않은 범위" 로 남겼던 `MobileLayout.tsx`·`RateTableViewer.tsx` 도 이제 테스트가 있다.
>
> **아직 열려 있는 것 2건:**
>
> 1. 🔴 **`SELECT count(*) FROM quotes WHERE fsc_percent = 0`** (HIGH-1 잔여 ②) — **Render DB 접근이 없어 미확인.** 해당 견적들은 화면에 약 24.7% 높은 금액이 표시되고 저장은 낮은 금액으로 됐다. 0건이면 종이 문제로 끝나므로 **규모 확인이 먼저**다. 이 문서에서 유일하게 고객 금액이 걸린 항목.
> 2. **`bundler-audit` CI 게이트 미도입** — 정책 판단. 2026-08-28 에 emax 가 `mail` CVE 를 7일간 방치한 게 정확히 이 게이트가 막았을 사례다.
>
> **의도적으로 남긴 것**(재논의 방지): HIGH-1 잔여 ③ 비교 카드 FSC 비대칭, 그리고 HIGH-1 잔여 ①(FE 상수 ↔ BE DB FSC 정합)은 `9a9063c5` 로 성격이 바뀌었다 — FE 가 이제 DB 를 먼저 읽고 상수는 폴백이라 "둘이 다른 값" 이 아니라 "폴백이 얼마나 묵었나" 의 문제다.
...
## MEDIUM-2 — `mail` 2.9.0 이메일 스푸핑 취약점 (신규) — ✅ **해소 (2026-08-28)**

> 락이 `mail (2.9.1)`, `bundler-audit check` **취약점 0건**.
>
> ⚠️ **smart-quote-emax 는 이때 함께 고쳐지지 않아 2.9.0 인 채로 7일 남아 있었다** — 2026-08-28 에 발견해 같이 닫았다(emax 도 0건). 이 문서가 "emax 도 볼 것" 이라고 적어 둔 것만으로는 실제로 적용되지 않았다는 뜻이다. **같은 스택 두 저장소는 감사도 두 번 돌릴 것.**
...
## MEDIUM-3 — 백엔드 입력 검증이 프론트 Zod 보다 약하다 (05-15 Phase B 미완) — ✅ **해소**

> `QuotesController::NUMERIC_INPUT_BOUNDS` + `validate_numeric_bounds!` 로 미러됨. `exchangeRate` >0·≤10,000, `fscPercent` 0..200, 나머지 6개 금액 필드 ≥0. calculate·create·파트너 3경로 공통이며 `NUMERIC_INPUT_PATTERN` 이 `"abc"`·`""` 를 숫자 이전 단계에서 거른다. 위 표의 `fscPercent: 100000` → 840,196,400원 경로는 이제 422 INVALID_INPUT.
...
## LOW-4 — `GET /auth/magic_link/verify` 라우트 잔존 — ✅ **해소**

> `config/routes.rb` 에 `post "auth/magic_link/verify"` 와 `Rails.env.test?` 가드가 걸린 `get .../peek` 만 남았다. 토큰이 URL 에 실리는 경로 제거됨.
...
## LOW-5 — `XK`(코소보)는 어느 캐리어에서도 견적이 불가능한 드롭다운 항목 — ✅ **해소 (드롭다운 제거 아님)**

> 처방으로 적어 둔 "매핑을 넣거나 드롭다운에서 빼기" 대신 **세 번째 선택지**를 택했다: `UNSERVICEABLE_COUNTRY_CODES` 를 **세 존 맵에서 파생**시켜 전용 안내 문구를 붙인다. 하드코딩 목록이 아니라 파생이라 존 테이블이 갱신되면 자동으로 따라온다.
>
> 판별의 핵심은 `unserviceable_countries.test.ts` 5건이 지키는 구분이다 — **한 캐리어라도 닿는 국가는 이 목록에 들어가면 안 된다**(예: `CW` 는 DHL 만 되므로 "캐리어를 바꿔 보라" 가 유효한 조언이다). 전부 안 되는 국가에만 그 조언이 막다른 길이 된다.
...
- ~~**Rails 8.0.5.1** (`~> 8.0.4` 핀). 직전 분석의 "EOL 2026-10-07" 은 **여전히 1차 출처 미확인**~~ → ✅ **2026-08-28 확인 완료. 그 날짜는 틀렸다.** `rubyonrails.org/maintenance` 1차 출처:
  - 8.0.x **버그픽스 지원은 2026-05-07 에 이미 끝나 있었다** — 이 분석을 쓴 08-21 시점에 3.5개월 경과. 아무도 그걸 보고 있지 않았다.
  - 8.0.x **보안** 지원 종료는 **2026-11-07** (인용돼 온 `2026-10-07` 은 월·일이 뒤바뀐 값으로 보인다).
  - **의심을 유지한 것 자체는 옳았다** — 실제로 두 군데가 틀렸다. 다만 "미확인이니 인용하지 말 것" 으로 두 번 넘긴 사이 버그픽스 지원이 끝난 채 굴러갔다. **날짜 하나 확인하는 데 웹 조회 한 번이면 됐다.**
  - 조치: 두 저장소 모두 `~> 8.1.3` → 락 **8.1.3.1**(보안 2027-10-10 까지). `config.load_defaults 8.0` 은 유지 — 젬 버전만 올리고 프레임워크 동작은 고정해 회귀면을 좁혔다. main 589건·emax 327건 전부 green, rubocop 0 offense, **신규 Rails deprecation 0건**.
  - ⚠️ 8.1.x 의 **버그픽스** 지원은 2026-10-10 까지다(보안은 2027-10-10). 다음 시리즈가 나오면 다시 올려야 한다.
